...
The internet is a big place, and many different people utilize the internet in everyday life. With so many people using the internet, it is important to consider _how_ users interact with the internet. Specifically, how individuals with disabilities interact and use the internet.

There are many different disabilities to consider, include auditory, cognitive, physical, speech and visual disabilities. Even situational disabilities come into play, a surgery, broken arm, concussion, baby, et al.

It's our responsibility as web developers to think of our users and design an accessible experience for as many users as possible.

## What

HTML elements have _semantic_ meaning that is designed by specifications to make our web pages accessible. The first step we can take to making our web pages more accessible is by using _good_ markup practices.

Some of the benefits of using _good_ markup practices are:

1. **Utilizing default element behaviors** — using the correct elements for the job also means that you get default functionality (a button is tabbable, a link turns your cursor to a pointer on hover, etc)
2. **Smaller file size and mobile friendly design** — using correct HTML semantics generally decreases the amount of HTML that is required for a page
3. **SEO optimization** — search engines give more importance to keywords inside headings, links, etc. than keywords included in non-semantic `<div>`s, etc., so your documents will be more findable by customers from search results
